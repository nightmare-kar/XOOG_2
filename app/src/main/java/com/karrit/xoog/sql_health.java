...
public class sql_health {
    ...
    public sql_health(Context ctx, String kids_id){
        this.course_id="health_type";
        this.ctx=ctx;
    ...
    public void download_next_task(final int level, final int Task){
        ...
        db.collection("health").document(Integer.toString(level)).collection(Integer.toString(Task)).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
...
*/
    ...
    public ArrayList<sport_exercise_class> readhealth(int level, int task){
